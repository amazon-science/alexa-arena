...
def get_first_available_gpu():
    deviceIDs = GPUtil.getFirstAvailable(order = 'first', maxLoad=0.5, maxMemory=0.5, attempts=1, interval=900, verbose=False)
    if not deviceIDs:
        raise ValueError("No GPUs available. Please check GPU usage and try again later.")
    return deviceIDs[0]

# def restore_checkpoint(path_to_model_checkpoint, optimizer=None, args=None):
...
def save_onnx_model(model, device, onnx_save_path):
    dummy_image = torch.randn(1, 3, 300, 300, device=device)
    # dummy_target = {
    #         'boxes':
    #             torch.tensor(
    #                 [[140., 127., 160., 175.],
    #                 [233.,  89., 259., 151.],
    #                 [234., 118., 258., 159.],
    #                 [257.,  64., 300., 147.]]).to(device),
    #         'labels':
    #             torch.tensor([ 51, 110,  12,   6]).to(device),
    #         'masks': torch.randn(4, 300, 300, device=device),
    #         'image_id': torch.tensor([308]).to(device),
    #         'area': torch.tensor([ 960., 1612.,  984., 3569.]).to(device),
    #         'iscrowd': torch.tensor([0, 0, 0, 0]).to(device)
    # }
    # dummy_input = (dummy_image, dummy_target)
    # model.eval()
    torch.onnx.export(
        model, dummy_image, onnx_save_path)
    print(f"Saving onnx model to {onnx_save_path}")
